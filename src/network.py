...
class Network():
    ...
    def SGD(self, training_data:np.ndarray, outputs:int=1, epochs:int=10, batch_size:int=10, eta:float=0.1, test_data:np.ndarray=None, train_monitoring=False):
        """
        ...
        for counter in range(epochs):
            ...
            for minibatch in mini_batches:
                ...
                f = lambda u, nu: u - (eta/batch_size) * nu
                self.weights = [f(w,nw) for w, nw in zip(self.weights, nabla_w_t)]
                self.biases = [f(b,nb) for b, nb in zip(self.biases, nabla_b_t)]
